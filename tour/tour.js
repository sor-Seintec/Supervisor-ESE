(()=>{
  ...
  const configs={
    ...
    'visita.html':[
      ...
      {selector:'#activityMode',title:'Informe a atividade',text:'Registre um acompanhamento simples ou selecione as ações cadastradas que foram realizadas.'},
    ...
    'kanban.html':[
      ...
      {selector:'.header-register',title:'Registrar visita',text:'Depois da atividade na escola, use este botão para confirmar o resultado da visita.'}
    ]
  };
  ...
  const currentSteps=()=> (configs[page]||configs['index.html']).map(item=>({...item,element:document.querySelector(item.selector)})).filter(item=>visible(item.element));
