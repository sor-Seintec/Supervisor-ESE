(()=>{
  ...
  const currentSteps=()=> (configs[page]||configs['index.html']).map(item=>({...item,element:document.querySelector(item.selector)})).filter(item=>item.element&&(item.prepare||visible(item.element)));
